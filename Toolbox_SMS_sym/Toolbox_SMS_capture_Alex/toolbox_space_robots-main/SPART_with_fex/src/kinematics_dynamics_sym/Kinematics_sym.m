function [RJ,RL,rJ,rL,e,g]=Kinematics_sym(R0,r0,qm,robot)
...
for i=1:n
    ...
    if cjoint.parent_link==0
        ...
        TJ{cjoint.id}(1:4,1:4)=TL{cjoint.parent_link}(1:4,1:4)*cjoint.T;
    end
    
    %Transformation due to current joint variable
    if cjoint.type==1
        %Revolute
        T_qm=[Euler_DCM(cjoint.axis,qm(cjoint.q_id))',SX.zeros(3,1);SX.zeros(1,3),1];
    elseif cjoint.type==2
        %Prismatic
        T_qm=[SX.eye(3),cjoint.axis*qm(cjoint.q_id);SX.zeros(1,3),1];
    else
        %Fixed
        T_qm=[SX.eye(3),SX.zeros(3,1);SX.zeros(1,3),1];
    end
    
    %Link Kinematics (homogeneous transformation matrix)
    clink=robot.links(cjoint.child_link);
    TL{clink.id}(1:4,1:4)=TJ{clink.parent_joint}(1:4,1:4)*T_qm*clink.T;
end
...
for i=1:n
    ...
    rJ(1:3,i)=TJ{i}(1:3,4);
    e(1:3,i)=RJ{i}(1:3,1:3)*robot.joints(i).axis;
end
%Link associated quantities
for i=1:n
    RL{i}=TL{i}(1:3,1:3);
    rL(1:3,i)=TL{i}(1:3,4);
    g(1:3,i)=rL(:,i)-rJ(1:3,robot.links(i).parent_joint);
end
