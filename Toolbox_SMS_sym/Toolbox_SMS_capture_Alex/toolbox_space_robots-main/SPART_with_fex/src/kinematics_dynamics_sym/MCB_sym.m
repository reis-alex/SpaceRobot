function [M0_tilde,Mm_tilde]=MCB_sym(I0,Im,Bij,Bi0,robot)
...
%Pre-allocate
for i=1:n
    Mm_tilde{i}=SX.sym('Mm_t',6,6);
end
%Backwards recursion
for i=n:-1:1
    %Initialize M tilde
    Mm_tilde{i}(1:6,1:6)=[Im{i},zeros(3,3);SX.zeros(3,3),robot.links(i).mass*SX.eye(3)];
    ...
    for j=1:length(child)
        ...
        Mm_tilde{i}(1:6,1:6)=Mm_tilde{i}(1:6,1:6)+Bij{child(j)}{i}(1:6,1:6)'*Mm_tilde{child(j)}(1:6,1:6)*Bij{child(j)}{i}(1:6,1:6);
    end
end
...
for j=1:length(child)
    M0_tilde=M0_tilde+Bi0{child(j)}(1:6,1:6)'*Mm_tilde{child(j)}(1:6,1:6)*Bi0{child(j)}(1:6,1:6);
end
