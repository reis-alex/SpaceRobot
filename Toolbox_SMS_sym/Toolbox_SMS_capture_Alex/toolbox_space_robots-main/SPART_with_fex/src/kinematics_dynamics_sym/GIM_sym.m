function [H0, H0m, Hm] = GIM_sym(M0_tilde,Mm_tilde,Bij,Bi0,P0,pm,robot)
import casadi.*
...
for j=1:n
    for i=j:n
        if robot.joints(i).type~=0 && robot.joints(j).type~=0
            Hm(robot.joints(i).q_id,robot.joints(j).q_id)=pm(1:6,i)'*Mm_tilde{i}(1:6,1:6)*Bij{i}{j}(1:6,1:6)*pm(1:6,j);
