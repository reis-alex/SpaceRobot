...
if isfield(opt.model,'parameters')
    for i = 1:length(opt.model.parameters)
        parameters{length(parameters)+1} = opt.model.parameters(i);
        aux = opt.model.parameters(i);
        opt.parameters.name = vertcat(opt.parameters.name, aux.name);
    end
end

% if perturbation is known

if ~isfield(opt,'input')
    opt.input.vector = [];
end

if isfield(opt.model,'perturbation')
    perturbation_vector = SX.sym('d',opt.N);
    for i = 0:opt.N-1
        parameters{length(parameters)+1} = perturbation_vector(i+1);
        opt.parameters.name = vertcat(opt.parameters.name, ['d_' int2str(i)]);
        opt.input.vector{length(opt.input.vector)+1} = {['d_' int2str(i)]};
    end
end
...
if ~isfield(opt.model,'states')
    ...
    if isfield(opt.model,'perturbation')
        perturbation = opt.model.perturbation;
    else
        perturbation = [];
    end
    if isfield(opt.model,'parameters')
        f = Function('f',{states,inputs,vertcat(opt.model.parameters{:}),vertcat(opt.model.perturbation)},{opt.model.function});
    else
        switch class(opt.model.function)
            case 'casadi.SX' 
                f = Function('f',{states,inputs},{opt.model.function});
            case 'casadi.Function'
                f = opt.model.function;
        end
    end
end
    ...
    switch opt.continuous_model.integration
        ...
        case 'euler'
            for k = 1:opt.N
                ...
                if isfield(opt.model,'parameters')
                    g               = [g; X(:,k+1)-(X(:,k)+opt.dt*f(X(:,k),U(:,k),vertcat(opt.model.parameters{:}),perturbation_vector(k)))];
                else
...
if isfield(opt.costs,'general')
    obj = obj + opt.costs.general.function(X,U,parameters_gnc{:});
end
...
if isfield(opt,'misc') && isfield(opt.misc,'optimize_ic')
    optimized_ic = X(opt.misc.optimize_ic,1);
    OPT_variables   = [optimized_ic;
                       reshape(X(:,2:end),opt.n_states*(opt.N),1);
...
if isfield(opt.constraints,'polyhedral') || ~isfield(opt,'constraints')
    ...
    args.ubx(length(args.ubx)+1:opt.n_states*(opt.N),1) = +inf;
        ...
        if isfield(opt,'misc') && isfield(opt.misc,'optimize_ic')
            args.lbx(length(args.lbx)+1:length(args.lbx)+length(opt.misc.optimize_ic),1) = opt.constraints.states.lower(opt.misc.optimize_ic);
            args.ubx(length(args.ubx)+1:length(args.ubx)+length(opt.misc.optimize_ic),1) = opt.constraints.states.upper(opt.misc.optimize_ic);
        end
    ...
    for jj = 1:length(opt.constraints.general.function)
        ...
        switch opt.constraints.general.elements{jj}
            ...
            case 'all'
                ...
                g = [g; opt.constraints.general.function{jj}(X,U,vertcat(parameters{:}))];
                %                 size_gc{jj} = size_gc{jj} +  length(opt.constraints.general.function{jj}(X,U,vertcat(parameters_gc{:})));
                size_gc{jj} = size_gc{jj} +  length(opt.constraints.general.function{jj}(X,U,vertcat(parameters{:})));
        end
    end
end
...
Param = [];
if isfield(opt,'misc') && isfield(opt.misc,'optimize_ic')
    if length(opt.misc.optimize_ic)~=length(opt.model.states)
        for i = 1:length(opt.model.states)
            if isempty(find(opt.misc.optimize_ic==i))
                Param = vertcat(Param, Init_states(i));
            end
        end
    end
else
    Param = [Init_states]; %[Init_states(opt.misc.optimize_ic)];
end
